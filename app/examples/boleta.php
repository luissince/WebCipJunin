...
if (!is_array($detalleventa)) {
    ...
    $cbc = $Invoice->appendChild($cbc);
    $cbc = $xml->createElement('cbc:ID', $ingreso->Serie . '-' . $ingreso->Numeracion);  // numero de factura
    $cbc = $Invoice->appendChild($cbc);
    ...
    $cbc = $Invoice->appendChild($cbc);
    $cbc = $xml->createElement('cbc:InvoiceTypeCode', $ingreso->TipoComprobante);
    $cbc = $Invoice->appendChild($cbc);
    ...
    $PartyIdentification = $xml->createElement('cac:PartyIdentification');
    $PartyIdentification = $cac_party->appendChild($PartyIdentification);
    $cbc = $xml->createElement('cbc:ID', $ingreso->NumeroDocumento);
    $cbc->setAttribute('schemeID', $ingreso->TipoDocumento);
    $cbc = $PartyIdentification->appendChild($cbc);
    $PartyLegalEntity = $xml->createElement('cac:PartyLegalEntity');
    $PartyLegalEntity = $cac_party->appendChild($PartyLegalEntity);
    $cbc = $xml->createElement('cbc:RegistrationName');
    $cbc->appendChild($xml->createCDATASection($ingreso->DatosPersona));
    $cbc = $PartyLegalEntity->appendChild($cbc);
    ...
    $cbc = $xml->createElement('cbc:LineExtensionAmount', number_format(round($totales['totalsinimpuesto'], 2, PHP_ROUND_HALF_UP), 2, '.', '')); //
    $cbc = $cac_LegalMonetaryTotal->appendChild($cbc);
    $cbc->setAttribute('currencyID', $tipomoneda);
    $cbc = $xml->createElement('cbc:TaxInclusiveAmount', number_format(round($totales['totalconimpuesto'], 2, PHP_ROUND_HALF_UP), 2, '.', ''));
    $cbc = $cac_LegalMonetaryTotal->appendChild($cbc);
    $cbc->setAttribute('currencyID', $tipomoneda);
    ...
    $filename = $empresa->NumeroDocumento . '-' . $ingreso->TipoComprobante . '-' . $ingreso->Serie . '-' . $ingreso->Numeracion;
