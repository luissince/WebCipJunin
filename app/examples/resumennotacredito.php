...
if (!is_array($detalleventa)) {
    ...
    $correlativo = ($correlativoActual === 0) ? (intval($notacredito->Correlativo) + 1) : ($correlativoActual + 1);
    ...
    $CustomerAssignedAccountID = $xml->createElement('cbc:CustomerAssignedAccountID', $notacredito->NumeroDocumento);
    $CustomerAssignedAccountID = $AccountingCustomerParty->appendChild($CustomerAssignedAccountID);
    $AdditionalAccountID = $xml->createElement('cbc:AdditionalAccountID', $notacredito->TipoDocumento); //
    ...
    if ($soapResult->isSuccess()) {
        ...
        if ($soapResult->isSuccess()) {
            if ($soapResult->isAccepted()) {
                NotaCreditoAdo::CambiarEstadoSunatResumen($idNotaCredito, $soapResult->getCode(),  $soapResult->getDescription(), $correlativo, $currentDate->format('Y-m-d'));
